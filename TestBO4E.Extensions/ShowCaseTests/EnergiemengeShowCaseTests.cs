...
public class EnergiemengeShowCaseTests
{
    ...
    public void ShowCaseTest()
    {
        ...
        Debug.WriteLine(
            $"You got Verbrauch data for {decimal.Round(em.GetCoverage() * 100.0M)}% of the time in between {em.Energieverbrauch.Select(v => v.Startdatum).Min():yyyy-MM-dd} and {em.Energieverbrauch.Select(v => v.Enddatum).Max():yyyy-MM-dd}");
        ...
        var consumptionMarch7 = em.GetConsumption(new TimeRange(
            new DateTimeOffset(2020, 3, 7, 0, 0, 0, TimeSpan.Zero).UtcDateTime,
            new DateTimeOffset(2020, 3, 8, 0, 0, 0, TimeSpan.Zero).UtcDateTime));
        Debug.WriteLine(
            $"The total consumption on March 7 is {decimal.Round(consumptionMarch7.Item1)}{consumptionMarch7.Item2}");
