...
#pragma warning restore 618
                ...
                foreach (var assertion in assertions.Properties())
                    switch (assertion.Name)
                    {
                        ...
                        case "totalConsumption-2018":
                            Assert.AreEqual(assertion.Value,
                                em.GetConsumption(new TimeRange(new DateTime(2018, 01, 01), new DateTime(2019, 01, 01)))
                                    .Item1);
                            break;
                        case "load-kW":
                            foreach (var dateAssertion in ((JObject) assertions["load-kW"]).Properties())
                                if (DateTime.TryParseExact(dateAssertion.Name, "yyyyMMddHHmmss",
                                    ...
                                    Assert.IsTrue(false,
                                        "Please specify the load dates in the testfiles as yyyyMMddHHmmss");
                            break;
                        case "average":
                            try
                            {
                        ...
                        case "missingTimeRangeCount":
                            Assert.AreEqual(assertion.Value, em.GetMissingTimeRanges().Count,
                        ...
                        case "coverage201804-KWHPROGNOSE1234":
                            Assert.AreEqual(assertion.Value,
                                Math.Round(
                                    em.GetCoverage(GERMAN_APRIL_2018, Wertermittlungsverfahren.PROGNOSE, "1-2-3-4",
                                        Mengeneinheit.KWH), 4), $"{assertion.Name}: {boFile}");
                            break;
                        case "coverage201804-KWHMESSUNG5678":
                            Assert.AreEqual(assertion.Value,
                                Math.Round(
                                    em.GetCoverage(GERMAN_APRIL_2018, Wertermittlungsverfahren.MESSUNG, "5-6-7-8",
                                        Mengeneinheit.KWH), 4), $"{assertion.Name}: {boFile}");
                            break;
                        case "jointCoverage":
                            Assert.AreEqual(assertion.Value, Math.Round(em.GetJointCoverage(GERMAN_APRIL_2018), 4),
                                $"{assertion.Name}: {boFile}");
                            break;
                        case "isPure":
                            ...
                            if (!(bool) assertion.Value)
                            {
                                ...
                                Assert.AreEqual(em.Energieverbrauch.Count,
                                    pureEms.Select(x => x.Energieverbrauch.Count).Sum());
                ...
                if (em.IsPure() && em.IsExtensive() && em.GetTotalConsumption().Item1 != 0.0M)
                {
                    ...
                    Assert.AreEqual(Math.Round(targetValue, 12),
                        Math.Round(emNormalised.GetTotalConsumption().Item1, 12));
                }

                if (em.IsIntensive()) // test this once for one object. that's enough
                    Assert.ThrowsException<ArgumentException>(() => em.GetTotalConsumption(),
                        "It must not be allowed to add up intensive units.");
            }
        }
