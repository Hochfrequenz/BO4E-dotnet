    ...
    public class TestEncrypter
    {
        ...
        public void TestBOEncryption()
        {
            ...
            foreach (var testFile in files)
            {
                ...
                var bo = BoMapper.MapObject(boType, json);
                ...
                var x509CertPubl =
                    new X509Certificate2(X509Certificate.CreateFromCertFile("encrypterTests/publickey.cer"));
                ...
                var x509CertPubl2 =
                    new X509Certificate2(X509Certificate.CreateFromCertFile("encrypterTests/publickey2.cer"));
                EncryptedObject eoMultiple;
                using (var xasyEncMultiple = new X509AsymmetricEncrypter(new HashSet<X509Certificate2>
                    {x509CertPubl, x509CertPubl2})) // encrypter needs not private key!
                {
                ...
                using (var xasydecMultiple = new X509AsymmetricEncrypter(keyPair.Private))
                {
                    ...
                    Assert.AreEqual(expectedString, actualString2,
                        "Original and encrypted->decrypted object do not match!");
                }
