...
namespace BO4E
{
    ...
    public class BoEdiMapper
    {
        ...
        ///         </item>
        ///         <item>
        ///             <description>the corresponding EDIFACT value</description>
        ///         </item>
        ///         <item>
        ///             <description>
        ///                 objectValue iff no mapping was defined but objectValue is already a valid EDIFACT value (e.g.
        ///                 for "Landescode"s)
        ///             </description>
        ///         </item>
        ///     </list>
        ...
        public static string ToEdi(string objectName, string objectValue)
        {
            ...
            if (prop == null)
            {
                ...
                if (alternativField != null && clazz.GetField(alternativField) != null) return objectValue;
            ...
            {
                Logger.LogWarning(
                    $"No EdiClass defined for {objectName}. Too lazy? Returning original value {objectValue}"); // holds true for landescode.
                return objectValue;
            }
            ...
            foreach (var aef in annotatedEdiFields)
                if (aef.GetCustomAttributes<MappingAttribute>().Select(ma => ma.Mapping.FirstOrDefault())
                    .Any(matchCandidate => matchCandidate != null && matchCandidate.ToString() == objectValue))
                {
        ...
        {
            var type = o.GetType();
            if (!type.IsSubclassOf(typeof(BusinessObject)) && !type.IsSubclassOf(typeof(COM.COM)))
            ...
            foreach (var oProp in o.GetType().GetProperties().Where(p => p.GetValue(o) != null))
            {
                ...
                else if (originalType.IsGenericType && originalType.GetGenericTypeDefinition() == typeof(List<>))
                {
                    ...
                    {
                        var listItemEdiType = Assembly.GetExecutingAssembly()
                            .GetType($"{NamespacePrefix}.EDI.{originalListItemType.Name}Edi");
                        ...
                        else
                        {
                            throw new NotImplementedException(
                                $"There is no BO4E<-->EDIFACT mapping defined for {originalListItemType}.");
                        }
                    }
                    else
                    {
