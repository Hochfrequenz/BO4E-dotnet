...
public class TestJsonOrder
{
    ...
    /// </summary>
    static readonly HashSet<Type> IgnoreOrderTypes =
        new()
        {
    ...
    protected static void TestOrderFromAbstract(Type abstractBaseType)
    {
        ...
        var relevantTypes = typeof(BusinessObject)
            .Assembly.GetTypes()
            .Where(abstractBaseType.IsAssignableFrom);
        foreach (
            var relevantType in relevantTypes.Where(t =>
                !IgnoreOrderTypes.Contains(t) && !t.Name.Contains("Marktrolle")
            )
        )
        {
            ...
            {
                var systemTextIgnore = dtProperty
                ...
                var systemTextJsonOrderAttribute = dtProperty
                    .GetCustomAttributes<JsonPropertyOrderAttribute>()
                    .FirstOrDefault();
                systemTextJsonOrderAttribute
                    .Should()
                    .NotBeNull(
                        because: $"The property {dtProperty.Name} of {relevantType.Name} should have System.Text.JsonPropertyOrderAttribute"
                    );
                var newtonSoftJsonPropertyAttribute = dtProperty
                    .GetCustomAttributes<JsonPropertyAttribute>()
                    .FirstOrDefault();
                newtonSoftJsonPropertyAttribute
                    .Should()
                    .NotBeNull(
                        $"The property {dtProperty.Name} of {relevantType.Name} should have Newtonsoft.Json.JsonPropertyAttribute"
                    );
                ...
                var newtonsoftIgnore = dtProperty
                    .GetCustomAttributes<Newtonsoft.Json.JsonIgnoreAttribute>()
                    .FirstOrDefault();
                ...
                systemTextOrder
                    ...
                    .Be(
                        newtonsoftOrder,
                        because: $"System.Text Order and Newtonsoft Order of {relevantType.Name}.{dtProperty.Name} should be the same."
                    );
            }
        }
    }
