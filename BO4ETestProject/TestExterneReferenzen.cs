...
namespace TestBO4E
{
    ...
    public class TestExterneReferenzen
    {
        ...
        public void TestGettingAndSetting()
        {
            ...
            Assert.ThrowsException<System.ArgumentException>(() => marktlokation.SetExterneReferenz(new ExterneReferenz { ExRefName = null, ExRefWert = "nicht bar" }), "must not add invalid values");
            Assert.ThrowsException<System.ArgumentException>(() => marktlokation.SetExterneReferenz(new ExterneReferenz { ExRefName = "foo", ExRefWert = null }), "must not add invalid values");
            ...
            marktlokation.SetExterneReferenz(new ExterneReferenz { ExRefName = "foo", ExRefWert = "bar" }); // if null, list is automatically created
            marktlokation.SetExterneReferenz(new ExterneReferenz { ExRefName = "foo", ExRefWert = "bar" }); // setting a non-conflicting value twice doesn't hurt
            Assert.IsTrue(marktlokation.TryGetExterneReferenz("foo", out actualBar));
            Assert.AreEqual("bar", actualBar);
        }
    }
