...
{
    [TestMethod]
    public void TestProtobufDateTimeWorkaroundCOM()
    {
        TestProtobufDateTimeWorkaround(typeof(COM));
    }

    [TestMethod]
    public void TestUniqueProtobufMemberIdCOM()
    {
        TestUniqueProtobufMemberIdAbstract(typeof(COM));
    }

    [TestMethod]
    public void TestUniqueProtoIncludeTagsCOM()
    {
        TestUniqueProtoIncludeTagAbstract(typeof(COM));
    }

    [TestMethod]
    public void TestProtobufDateTimeWorkaroundBO()
    {
        TestProtobufDateTimeWorkaround(typeof(BusinessObject));
    }

    [TestMethod]
    public void TestUniqueProtobufMemberIdBO()
    {
        TestUniqueProtobufMemberIdAbstract(typeof(BusinessObject));
    }

    [TestMethod]
    public void TestUniqueProtoIncludeTagsBO()
    {
        TestUniqueProtoIncludeTagAbstract(typeof(BusinessObject));
    }
    ...
    protected void TestUniqueProtobufMemberIdAbstract(Type abstractType)
    {
        if (!abstractType.IsAbstract)
        {
            throw new ArgumentException(
                $"The type {abstractType} is not abstract",
                nameof(abstractType)
            );
        }
        ...
        foreach (
            var type in typeof(BusinessObject)
                .Assembly.GetTypes()
                .Where(t => abstractType.IsAssignableFrom(t))
        )
    ...
    protected void TestProtobufType(Type type, bool isDirectBase)
    {
        ...
        if (isDirectBase)
        {
            ...
            fieldsWithProtoMemberAttribute = type.GetFields(
                    BindingFlags.Public | BindingFlags.Instance
                )
                .Where(f => f.GetCustomAttributes(typeof(ProtoMemberAttribute), true).Any());
        }
        else
        {
            ...
            fieldsWithProtoMemberAttribute = type.GetFields(
                ...
                .Where(f => f.GetCustomAttributes(typeof(ProtoMemberAttribute), false).Any());
        }

        var nonOfficialFieldsWithProtoMember = allFields
            .Where(field => field.GetCustomAttributes(typeof(NonOfficialAttribute)).Any()) // those fields having an [NonOfficial(...)] attribute
            .Where(field =>
                (
                    (NonOfficialAttribute)
                        field.GetCustomAttributes(typeof(NonOfficialAttribute)).First()
                ).HasCategory(NonOfficialCategory.CUSTOMER_REQUIREMENTS)
            ) // and the customer_requirements category
        ...
        var wrongTagsNonOfficial = nonOfficialFieldsWithProtoMember.Where(f =>
            ((ProtoMemberAttribute)f.GetCustomAttributes(typeof(ProtoMemberAttribute)).First()).Tag
            < 1000
        );
        Assert.AreEqual(
            0,
            wrongTagsNonOfficial.Count(),
            $"Fields in {type} are non official and do not have proto tags >= 1000: {string.Join(", ", wrongTagsNonOfficial.Select(f => f.Name))}"
        );
        var wrongTagsOfficial = fieldsWithProtoMemberAttribute
            .Except(nonOfficialFieldsWithProtoMember)
            .Where(f =>
                (
                    (ProtoMemberAttribute)
                        f.GetCustomAttributes(typeof(ProtoMemberAttribute)).First()
                ).Tag > 1000
            );
        Assert.AreEqual(
            0,
            wrongTagsOfficial.Count(),
        ...
        try
        {
            if (isDirectBase)
            {
                Assert.AreEqual(
                    allFields.Length,
                    fieldsWithProtoMemberAttribute.Count(),
        ...
        catch (ArgumentOutOfRangeException aoore) when (aoore.ParamName == "tag")
        {
            Assert.IsTrue(
                false,
                $"Do you have any ProtoMember attributes with an id<=0 in {type}?"
            );
            throw;
        }
        ...
        if (isDirectBase) // because protobuf-net doesn't support multiple levels of inheritance
        {
            ...
            Assert.AreEqual(
                0,
                duplicateTags.Count,
                $"The following [(ProtoMember(<tag>)] tags in {type} are not unique: {string.Join(", ", duplicateTags.Keys)}"
            );
        }
        else
        {
            Assert.AreEqual(
                0,
                fieldsWithProtoMemberAttribute.Count(),
                $"Classes like {type} not directly inheriting from BusinessObject or COM must not have Protobuf Attributes (due to protobuf-net bugs)"
            );
        }
    }
    ...
    protected void TestProtobufDateTimeWorkaround(Type abstractBaseType)
    {
        ...
        foreach (var relevantType in relevantTypes)
        {
            ...
            foreach (var dtProperty in dtProperties)
            {
                ...
                var cla = dtProperty
                    .GetCustomAttributes<CompatibilityLevelAttribute>()
                    .FirstOrDefault();
                Assert.AreEqual(
                    CompatibilityLevel.Level240,
                    cla?.Level,
                    $"The property {dtProperty.Name} of type {relevantType.Name} does not have the Compatability Level Attribute or the wrong value"
                );
            }

            var nullableDtProperties = relevantType
                .GetProperties()
                .Where(p =>
            ...
            foreach (var nullableDtProperty in nullableDtProperties)
            {
                ...
                var pia = nullableDtProperty
                    .GetCustomAttributes<ProtoIgnoreAttribute>()
                    .FirstOrDefault();
                if (
                    relevantType == typeof(Zeitraum)
                    && new HashSet<string> { "startzeitpunkt", "endzeitpunkt" }.Contains(
                        nullableDtProperty.Name.ToLower()
                    )
                )
                {
                    continue;
                ...
                Assert.IsTrue(
                    relevantType
                        ...
                        .Any(p =>
                            p.GetCustomAttributes<ProtoMemberAttribute>()
                                .Any(pma => pma.Name == nullableDtProperty.Name)
                            && p.GetCustomAttributes<System.Text.Json.Serialization.JsonIgnoreAttribute>()
                                .Any()
                            && p.GetCustomAttributes<Newtonsoft.Json.JsonIgnoreAttribute>().Any()
                            && p.GetCustomAttributes<CompatibilityLevelAttribute>()
                                .Any(cla => cla.Level == CompatibilityLevel.Level240)
                        ),
    ...
    protected void TestUniqueProtoIncludeTagAbstract(Type abstractBaseType) // ToDo: test this with preisblatt
    {
        ...
        var duplicateIncludeTags = typeof(BusinessObject)
            .Assembly.GetTypes()
            .Where(t => abstractBaseType.IsAssignableFrom(t))
        ...
        Assert.AreEqual(
            0,
            duplicateIncludeTags.Count,
            $"The following [(ProtoInclude(<tag>, ...)] attributes are not unique: {string.Join(", ", duplicateIncludeTags.Keys)}"
        );
    }
    ...
    public void TestProtoEnumConsistency() //https://github.com/protobuf-net/protobuf-net/issues/60
    {
        ...
        {
            var naturalEnumValues = enumType
                .GetFields()
                .Where(f => !f.GetCustomAttributes<ProtoEnumAttribute>().Any())
            ...
            Assert.IsTrue(
                Enum.IsDefined(enumType, 0),
                $"Any enum must define a ZERO like value for Protobuf3 but {enumType} doesn't."
            );
        }
        ...
        Assert.IsFalse(
            nonDistinctValues.Any(),
            $"There are non-distinct Enum values. Add a matching [ProtoEnum] attribute to: {JsonConvert.SerializeObject(nonDistinctValues)}"
        );
    }
    ...
    {
        var businessObjectTypes = typeof(BusinessObject)
            .Assembly.GetTypes()
        ...
        foreach (var typePair in boTypesCrossProduct.Distinct())
        {
            ...
            if (typePair.inheritingType.BaseType == typeof(BusinessObject)) //because protobuf-net doesn't support mutliple levels of inheritance
            {
                Assert.IsTrue(
                    typePair
                        .inheritingType.GetCustomAttributes(typeof(ProtoContractAttribute), false)
                        .Any(),
            ...
            else
            {
                Assert.IsFalse(
                    typePair
                        .inheritingType.GetCustomAttributes(typeof(ProtoContractAttribute), false)
                        .Any(),
                    $"The (INDIRECTLY inheriting) type {typePair.inheritingType} has the [ProtoContract] attribute."
                ); // bug in protobuf-net
            }

            if (
                typePair.inheritingType.BaseType == typePair.baseType
            ...
            )
            {
                ...
                var typesReferencedByBase = typePair
                    .baseType.GetCustomAttributes(typeof(ProtoIncludeAttribute), false)
                    .Cast<ProtoIncludeAttribute>()
                    .Select(pia => pia.KnownType);
                Assert.IsFalse(
                    typesReferencedByBase.Contains(typePair.inheritingType),
                    $"The base type {typePair.baseType} does not reference the inheriting type {typePair.inheritingType} in a [ProtoInclude(<tag>, {typePair.inheritingType})] attribute."
                );
            ...
            Assert.IsTrue(
                typePair.baseType.GetCustomAttributes(typeof(ProtoIncludeAttribute), false).Any(),
                $"The base type {typePair.baseType} must refer to the inheriting type {typePair.inheritingType} using a [ProtoInclude] attribute."
            );
