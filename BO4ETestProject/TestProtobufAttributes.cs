    ...
    public class TestProtobufAttributes
    {
        ...
        protected void TestProtobufType(Type type, bool isDirectBase)
        {
            ...
            if (isDirectBase)
            {
                ...
                fieldsWithProtoMemberAttribute = type.GetFields(BindingFlags.Public | BindingFlags.Instance)
            ...
            else
            {
                ...
                fieldsWithProtoMemberAttribute = type
                    .GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Where(f =>
                        f.GetCustomAttributes(typeof(ProtoMemberAttribute), false).Any());
            }
            ...
            if (isDirectBase) // because protobuf-net doesn't support multiple levels of inheritance
            {
                ...
                Assert.AreEqual(0, duplicateTags.Count,
                    $"The following [(ProtoMember(<tag>)] tags in {type} are not unique: {string.Join(", ", duplicateTags.Keys)}");
            }
            else
            {
                Assert.AreEqual(0, fieldsWithProtoMemberAttribute.Count(),
        ...
        protected void TestUniqueProtoIncludeTagAbstract(Type abstractBaseType) // ToDo: test this with preisblatt
        {
            ...
            Assert.AreEqual(0, duplicateIncludeTags.Count,
        ...
        public void TestProtoEnumConsistency() //https://github.com/protobuf-net/protobuf-net/issues/60
        {
            ...
            {
                var naturalEnumValues = enumType.GetFields()
                ...
                Assert.IsTrue(Enum.IsDefined(enumType, 0),
            ...
            Assert.IsFalse(nonDistinctValues.Any(),
        ...
        {
            var businessObjectTypes = typeof(BusinessObject).Assembly.GetTypes()
            ...
            foreach (var typePair in boTypesCrossProduct.Distinct())
            {
                    ...
                    typePair.baseType != typeof(BusinessObject))
                {
                    ...
                    Assert.IsFalse(typesReferencedByBase.Contains(typePair.inheritingType),
                ...
                Assert.IsTrue(typePair.baseType.GetCustomAttributes(typeof(ProtoIncludeAttribute), false).Any(),
                ...
                if (typePair.inheritingType.BaseType == typePair.baseType)
                {
                    var typesReferencedByBase = typePair.baseType
                        .GetCustomAttributes(typeof(ProtoIncludeAttribute), false).Cast<ProtoIncludeAttribute>()
                        .Select(pia => pia.KnownType);
                    Assert.IsTrue(typesReferencedByBase.Contains(typePair.inheritingType),
                        $"The base type {typePair.baseType} does not reference the inheriting type {typePair.inheritingType} in a [ProtoInclude(<tag>, {typePair.inheritingType})] attribute.");
                }
            }
        }
